  ...
  request(url, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      for (const activity of activities) {
        ...
        if (newObject.reporting_org.narrative)
          newObject.reporting_org.narrative =
            newObject.reporting_org.narrative[0];
        if (newObject.title) newObject.title = newObject.title[0];
        if (newObject.description)
          newObject.description = newObject.description[0];

        newObject = formatObjectFurther(newObject, activity);
        data.push(newObject);
      }
...
/* GET IATI Activity data with related fields. */
    ...
    if (!error && response.statusCode == 200) {
      ...
      for (const activity of activities) {
        ...
        let newObject = {
          ...
          related_activity: {
            ref: activity.related_activity_ref,
            type: activity.related_activity_type,
          },
          title: activity.title_narrative,
          description: activity.description_narrative,
          participating_orgs: [],
        ...
        if (newObject.related_activity.ref) {
          newObject.related_activity.ref = newObject.related_activity.ref[0];
          newObject.related_activity.type = newObject.related_activity.type[0];
        }
        // participating orgs
        newObject = formatObjectFurther(newObject, activity);
        data.push(newObject);
...
const formatObjectFurther = (dataObject, activity) => {
